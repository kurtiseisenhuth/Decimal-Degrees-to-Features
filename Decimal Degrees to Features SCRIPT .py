...
# set dataframe option to display all rows
...
def gar_to_dd(gar):
    try:
        # extract UTM zone, hemisphere, easting, and northing from GAR
        zone = gar[0:2]
        hemisphere = gar[2]
        easting = gar[3:8]
        northing = gar[8:]
        ...
        easting_meters = float(easting) * 1000.0
        northing_meters = float(northing) * 1000.0

        # example conversion using pyproj
        in_proj = Proj(proj='utm', zone=int(zone), datum='WGS84', hemisphere='north' if hemisphere == 'N' else 'south')
        out_proj = Proj(proj='latlong', datum='WGS84')
        lon, lat = transform(in_proj, out_proj, easting_meters, northing_meters)
        return lat, lon

    except Exception as e:
        print(f"Error processing {gar}: {e}")
        return float('nan'), float('nan')

def convert_wgrs_to_decimal(wgrs_coordinates):
    # convert World Geographic Reference System (WGRS) to decimal degrees
    def wgrs_to_dd(wgrs):
        # extract UTM zone from WGRS
        zone_match = re.match(r'(\d+)', wgrs)
        if not zone_match:
            return float('nan'), float('nan')  # Invalid WGRS format

        zone = zone_match.group()

        # example conversion using pyproj
        in_proj = Proj(proj='utm', zone=int(zone), datum='WGS84')
        out_proj = Proj(proj='latlong', datum='WGS84')
        lon, lat = transform(in_proj, out_proj, wgrs[len(zone):])
        return lat, lon

    lat_lon = wgrs_coordinates.apply(wgrs_to_dd)
...
def save_to_shapefile(df, shapefile_path):
    ...
    temp_csv_path = 'C:/Users/JJCT966/OneDrive - Tennessee/Desktop/TDOT Projects/CoordinateCleanerConverter/Universal Coordinate Test/temp.csv'  # Set the appropriate temporary path
...
convert_coordinates('C:/Users/JJCT966/OneDrive - Tennessee/Desktop/TDOT Projects/CoordinateCleanerConverter/Universal Coordinate Test/dms_coordinates.csv',
                    'C:/Users/JJCT966/OneDrive - Tennessee/Desktop/TDOT Projects/CoordinateCleanerConverter/Universal Coordinate Test/OutputTest.csv',
                    'C:/Users/JJCT966/OneDrive - Tennessee/Desktop/TDOT Projects/CoordinateCleanerConverter/Universal Coordinate Test/OutputTest.shp', 'dms')
